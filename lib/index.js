...
class Authenticator {
  ...
  authorized (filter) {
    return async function (req, res, next) {
      ...
      console.log("req.authentication.claims==>>", req.authentication.claims);
      console.log("await filter(req.authentication.claims)==>>", await filter(req.authentication.claims));
